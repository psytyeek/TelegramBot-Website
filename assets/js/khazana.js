...
showChapter.addEventListener('click', function handleShowChapterClick() {
    ...
    VideoData.data.slice().reverse().forEach(function (lecture) {
        lecture.content.forEach(function (item) {
            if (!item.hasOwnProperty('videoDetails')) {
                ...
                if (lecture.hasOwnProperty('topicDetails') && lecture.topicDetails.hasOwnProperty('name')) {
                    var chapterName = lecture.topicDetails.name;
        ...
        if (Note !== "") {
            ...
            NoteData.data.slice().reverse().forEach(function (item, index) {
                if (Video === "") {
                    if (!item.hasOwnProperty('videooDetails')) {
                        var ch_img = "https://www.pw.live/study/assets/banner.png";
                        if (chapterName === "") {
        ...
        const data = {
            ...
            chapter: document.getElementById('Chapter_Name').textContent,
            data: {
                videoData: Video,
                noteData: Note,
                dppData: Dpp
...
function pcDownloadHandler(data) {
    ...
    fetch("https://muftukmallbot.koyeb.app/khazana", {
        method: "POST",
